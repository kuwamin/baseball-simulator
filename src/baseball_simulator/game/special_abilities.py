...
def calculate_pitcher_special_ability(
    pitcher: Player, batter: Player, is_risp: bool
) -> tuple[int, int, int]:
    ...
    p_special = pitcher.pitcher.ability.special_ability
    ...
    if is_risp:
        s, c, b = PITCHER_RISP_MAP.get(p_special.clutch_pitching, (0, 0, 0))
        speed_corr += s
        control_corr += c
    ...
    if b_info.dominant_hitting == "左":
        s, c, b = PITCHER_LEFT_MAP.get(p_special.vs_left_batter, (0, 0, 0))
        speed_corr += s
        control_corr += c
        breaking_ball_corr += b

    # ノビ (fastball_life)
    speed_corr += PITCHER_NOBI_MAP.get(p_special.fastball_life, 0)
